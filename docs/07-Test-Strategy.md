...
- `test_create_product_writes_sql_and_vector` (CAT-1, CAT-4): assert row exists in SQL **and**
  a query against Chroma returns it
- `test_vector_write_failure_sets_synced_false` (CAT-4, NFR-3): mock Chroma failure, assert
  `vector_synced=False` and product still readable from SQL (no data loss, no silent success)
- `test_delete_product_removes_from_vector_store` (CAT-3)
...
- `test_retrieval_returns_only_indexed_products` (AGT-3)
...
- `test_narrative_ids_are_filtered_against_candidates` (AGT-5) — feed the mocked LLM a response
  containing a product ID that was **not** in the candidate set; assert it's dropped before storage.
